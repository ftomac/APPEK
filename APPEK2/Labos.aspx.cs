...
namespace APPEK2
{
    public partial class Labos : System.Web.UI.Page
    {
        ...
        public string DohvatiNaziv(int serial)
        {
            ...
            if (kemikalija.Read())
                naziv = String.Format("{0}", kemikalija["chemName"]);
        ...
        protected void btnUzmi_Click(object sender, EventArgs e)
        {
            ...
            int unosKolicine = int.Parse(txtKom.Text);
            ...
            else
            {
                ...
                if (kemikalija.Read())
                {
                    ...
                    dbConnect.Insert(String.Format("INSERT INTO Dashboard (note, date, ID_prijava) VALUES ('{0} je uzeo/la {1} komad/a {2} iz {3}', '{4}', {5})",
        ...
        protected void btnDodaj_Click(object sender, EventArgs e)
        {
            ...
            if(provjeriSkladiste.Read())
                ...
                else
                {
                    ...
                    connect.Update(String.Format("UPDATE {0} SET quantity = {1} WHERE ID_chemical = {2}", 
                                                  Session["odabraniLab"].ToString(), 
                                                  novoStanje, 
                                                  int.Parse(Session["idChem"].ToString())));
                    ...
                    connect.Insert(String.Format("INSERT INTO Dashboard (note, date, ID_prijava) VALUES ('Dodano je novih {0} komad/a {1} u {2}', '{3}', {4})",
                                                  unosKolicine,
        ...
        protected void btnPrenesi_Click(object sender, EventArgs e)
        {
            ...
            {
                DBConnect chemName = new DBConnect();
                MySqlDataReader kemikalija = chemName.Select("SELECT chemName FROM Skladiste WHERE ID_chemical = " + idChemSkl);
                ...
                if (kemikalija.Read())
                {
                    nazivChem = String.Format("{0}", kemikalija["chemName"]);
                }
                ...
                if (postoji)
                {
                    dbConnect.Update(String.Format("UPDATE {0} SET quantity = {1} WHERE ID_chemical = {2}", 
                                                    Session["odabraniLab"].ToString(), 
                                                    quantityLab+quantity, idChemSkl));
                    ...
                    dbConnect.Insert(String.Format("INSERT INTO Dashboard (note, date, ID_prijava) VALUES ('{0}', '{1}', {2})",
                                                    "Doneseno je " + quantity + " komad/a " + nazivChem + " u " + Session["OdabraniLab"] + " ",
                                                    FormatDateForMySQL(DateTime.Now),
                                                    korisnik.IdKorisnik));
                }
                else
                {
                    ...
                    dbConnect.Insert(String.Format("INSERT INTO Dashboard (note, date, ID_prijava) VALUES ('{0}', '{1}', {2})",
                                                    "Preneseno je " + quantity + " komad/a " + nazivChem + " iz skladišta u " + Session["OdabraniLab"] + " ",
                                                    FormatDateForMySQL(DateTime.Now),
                                                    korisnik.IdKorisnik));
                }
